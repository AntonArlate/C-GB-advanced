...
// структура для списка выделеной памяти
typedef struct list
{
    void *address;
    size_t size;
...
// печать списка. Либо сообщение об отсутствии утечек
...
// замена стандартной malloc()
void *my_malloc(size_t size, const char *file, int line, const char *func)
{
    void *ptr = malloc(size);
    char coment[64] = {0};
    sprintf(coment, "Allocated = %s, %i, %s, %p[%li]", file, line, func, ptr,
            size);
    insert(&memlist, ptr, size, coment);
    return ptr;
}

// замена стандартной free()
